...
export default function Navbar({
  ...
  const tabs = [
    ...
    { id: "classifieds", label: "استخدام و نیازمندی", icon: Briefcase, show: true },
  ];
              ...
              {user ? (
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative" ref={dropdownRef}>
                    ...
                    {showDropdown && (
                      <div className="absolute left-0 top-full mt-2 w-72 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 flex flex-col py-2 text-right animate-in fade-in slide-in-from-top-2 duration-150">
                        {/* Profile Header */}
                        <div className={`px-4 py-3 border-b border-slate-100 mb-1 flex items-center justify-between ${isAdmin ? "bg-rose-50/80" : "bg-slate-50/70"}`}>
                          <div>
                            <p className="text-xs font-extrabold text-slate-800">{profile?.name || (isAdmin ? "مدیر ارشد سیستم" : user.email?.split("@")[0])}</p>
                            <p className="text-[10px] text-slate-500 font-mono mt-0.5">{profile?.phone || user.email || "حساب دسترسی"}</p>
                          </div>
                          {isAdmin && (
                            <span className="text-[9px] font-extrabold bg-rose-600 text-white px-2 py-0.5 rounded-md shadow-xs">
                              Super Admin
                            </span>
                          )}
                        </div>
                        
                        {/* If Admin: Direct Admin Panel Controls (Admins have NO personal workshop, they only manage & supervise) */}
                        {isAdmin ? (
                          <div className="px-2 py-1 space-y-1">
                            <div className="px-2 py-1 bg-amber-50/80 border border-amber-200/60 rounded-xl mb-2">
                              <p className="text-[10px] font-bold text-amber-800 leading-relaxed">
                                🔒 شما به عنوان مدیر ارشد وارد شده‌اید (دارای دسترسی نظارتی و بدون کارگاه شخصی).
                              </p>
                            </div>
                            ...
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                                activeTab === "admin"
                                  ...
                                  : "bg-rose-50/50 text-rose-800 hover:bg-rose-100/70 border border-rose-100"
                              }`}
                            >
                              <div className="flex items-center gap-2.5">
                                <div className={`p-1.5 rounded-lg ${activeTab === "admin" ? "bg-white/20 text-white" : "bg-rose-200 text-rose-700"}`}>
                                  <Settings className="h-4 w-4" />
                                </div>
                                <div className="flex flex-col text-right">
                                  <span className="font-extrabold">ورود به پنل مدیریت ارشد</span>
                                  <span className={`text-[10px] ${activeTab === "admin" ? "text-rose-100" : "text-rose-600/80"}`}>
                                    نظارت بر کارگاه‌ها، پیامک و دیتابیس
                                  </span>
                                </div>
                              </div>
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${activeTab === "admin" ? "bg-white/20 text-white" : "bg-white text-rose-700 border border-rose-200"}`}>
                                مدیریت
                              </span>
                            </button>
                          </div>
